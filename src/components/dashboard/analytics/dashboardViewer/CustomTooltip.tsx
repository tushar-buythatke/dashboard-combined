        ...
        <div
            className={cn(
                ...
                isPinned
                    ...
                    : "rounded-[10px] backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.35)] border-l-[3px] px-3.5 py-2.5 min-w-[300px] max-w-[500px]"
            )}
            style={!isPinned ? {
        ...
        >
            ...
            <div className="relative z-10">
                ...
                <div className={cn(
                    ...
                    isPinned ? "border-b border-slate-200/60 dark:border-slate-700/60" : "border-b border-white/10"
                )}>
                    <div className="flex items-center gap-2 md:gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                            <div className={cn("font-bold text-sm md:text-base leading-tight truncate", isPinned ? "text-foreground" : "text-white")}>{label}</div>
                            <div className={cn("text-[10px] md:text-[11px] font-medium mt-0.5", isPinned ? "text-muted-foreground" : "text-white/55")}>{eventDataItems.length} {itemLabel}{eventDataItems.length !== 1 ? 's' : ''}</div>
                            {isPartialLatestDay && (
                        ...
                        <div
                            className={cn(
                                ...
                                isPinned
                                    ...
                                    : (someAvgEvents ? "text-amber-400" : "text-white")
                            )}
                        >
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2 md:gap-2.5 min-w-0 flex-1">
                                    ...
                                    />
                                    <span className={cn("text-xs md:text-sm font-bold break-words leading-tight", isPinned ? "text-foreground" : "text-white")}>{item.name}</span>
                                </div>

                                {/* Metric Display (Timing or Count) */}
                                {item.isAvgEvent && item.metricLabel ? (
                                    <div className="flex flex-col items-end">
                                        <span className="text-sm md:text-base font-extrabold text-amber-400 flex items-center gap-1">
                                            <Clock className="w-3 h-3" />
                                            {item.metricLabel}
                                        </span>
                                    </div>
                                ) : !item.isAvgEvent && (
                                    <span className={cn("text-sm md:text-base font-extrabold", isPinned ? "bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-200 bg-clip-text text-transparent" : "text-white")}>{formatMetric(item.count, 'count')}</span>
                                )}
                            </div>
                ...
                <div className={cn("mt-3 md:mt-4 pt-3 md:pt-4 border-t border-dashed", isPinned ? "border-gray-200 dark:border-gray-800" : "border-white/10")}>
                    <div className="flex items-center justify-between text-[11px] md:text-xs">
                        {someAvgEvents && formattedAvgMetric ? (
                            <span className={cn("font-medium flex items-center gap-1", isPinned ? "text-muted-foreground" : "text-white/55")}>
                                <Clock className="w-3 h-3 text-amber-500" />
                                Avg: <span className="font-extrabold text-amber-400">{formattedAvgMetric}</span>
                            </span>
                        ) : (
                            <span className={cn("font-medium uppercase tracking-wider opacity-60", isPinned ? "text-muted-foreground" : "text-white/55")}>Total Population</span>
                        )}

                        <div className="flex items-center gap-3">
                            <span className={cn("font-extrabold", isPinned ? "text-foreground" : "text-white")}>{formatMetric(totalCount, 'count')} <span className={cn("text-[9px] font-medium", isPinned ? "text-muted-foreground" : "text-white/45")}>reqs</span></span>
